...
namespace PW
{
    ...
    public class BewerageMaker : MonoBehaviour
    {
        ...
        private GameObject currentCupObject; // 현재 머신 위에 있는 컵
        ...
        {
            // 1. 이미 컵이 있고
            // 2. 컵 오브젝트가 파괴되지 않았으면 (null 아님)
            if (!canFillCup || currentCupObject != null)
                return;
        ...
        void SetTheCup()
        {
            // 부모를 지정하지 않고 Instantiate
            GameObject cup = Instantiate(cupType);

            // 위치와 회전만 cupSpot 기준으로 맞춤
            cup.transform.position = fillCupSpot.position;
            cup.transform.rotation = fillCupSpot.rotation;

            currentCupObject = cup;
        ...
        public void UnSetTheCup()
        {
            canFillCup = true;
            fillCupHelper = null;
            currentCupObject = null; // ⭐ 컵 오브젝트도 null 처리
            m_Collider.enabled = true;
        }
